...
dbapi.getVirtualDevices = function() {
  return db.run(r.table('virtual_devices'))
    .then(function(cursor) {
      return cursor.toArray()
    })
}

dbapi.getVirtualDevice = function(id) {
  return db.run(r.table('virtual_devices').get(id))
}

dbapi.createVirtualDevice = function(data) {
  const id = util.format('%s', uuid.v4()).replace(/-/g, '')
  const now = r.now()

  return db.run(r.table('virtual_devices').insert(Object.assign({}, data, {
    id: id
  , createdAt: now
  , updatedAt: now
  })))
    .then(function() {
      return dbapi.getVirtualDevice(id)
    })
}

dbapi.updateVirtualDevice = function(id, data) {
  return db.run(r.table('virtual_devices').get(id).update(
    Object.assign({}, data, {updatedAt: r.now()})
  ))
  .then(function() {
    return dbapi.getVirtualDevice(id)
  })
...
dbapi.deleteVirtualDevice = function(id) {
  return db.run(r.table('virtual_devices').get(id).delete())
}

dbapi.getAiTasks = function() {
  return db.run(r.table('ai_tasks'))
    .then(function(cursor) {
      return cursor.toArray()
    })
}

dbapi.getAiTasksBySerial = function(serial) {
  return db.run(r.table('ai_tasks').getAll(serial, {index: 'serial'}))
    .then(function(cursor) {
      return cursor.toArray()
    })
}

dbapi.getAiTask = function(id) {
  return db.run(r.table('ai_tasks').get(id))
}

dbapi.createAiTask = function(data) {
  const id = util.format('%s', uuid.v4()).replace(/-/g, '')
  const now = r.now()

  return db.run(r.table('ai_tasks').insert(Object.assign({}, data, {
    id: id
  , createdAt: now
  , updatedAt: now
  })))
    .then(function() {
      return dbapi.getAiTask(id)
    })
}

dbapi.updateAiTask = function(id, data) {
  return db.run(r.table('ai_tasks').get(id).update(
    Object.assign({}, data, {updatedAt: r.now()})
  ))
  .then(function() {
    return dbapi.getAiTask(id)
...
dbapi.deleteAiTask = function(id) {
  return db.run(r.table('ai_tasks').get(id).delete())
}

dbapi.getAiDeviceMemory = function(serial) {
  return db.run(r.table('ai_device_memory').get(serial))
}

dbapi.upsertAiDeviceMemory = function(serial, data) {
  ...
  return db.run(r.table('ai_device_memory').get(serial))
    .then(function(existing) {
      if (existing) {
        return db.run(r.table('ai_device_memory').get(serial).update(payload))
          .then(function() {
            return dbapi.getAiDeviceMemory(serial)
          })
      }
      return db.run(r.table('ai_device_memory').insert(Object.assign({}, payload, {
        createdAt: r.now()
      })))
        .then(function() {
          return dbapi.getAiDeviceMemory(serial)
...
dbapi.deleteAiDeviceMemory = function(serial) {
  return db.run(r.table('ai_device_memory').get(serial).delete())
}
