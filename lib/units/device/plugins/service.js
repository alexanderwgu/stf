...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, router, push, sdk, apk) {
    ...
    function callService(intent) {
      var preferredCmd = (sdk.level < 26) ? 'startservice' : 'start-foreground-service'
      log.info('using \'%s\' command for API %s', preferredCmd, sdk.level)

      function runAm(cmd, withUser) {
        var userArg = withUser ? '--user 0 ' : ''
        ...
        , intent
          ...
          .then(function(buffer) {
            ...
            var errorLine = lines.find(function(line) {
      ...
      function maybeRetryWithoutUser(err, cmd) {
        if (err && err.message && err.message.indexOf('--user') !== -1) {
          return runAm(cmd, false)
        }
        throw err
      }

      return runAm(preferredCmd, true)
        .catch(function(err) {
          return maybeRetryWithoutUser(err, preferredCmd)
        })
        .catch(function(err) {
          if (preferredCmd === 'start-foreground-service' &&
              err && err.message &&
              /Not found; no service started/i.test(err.message)) {
            log.warn('Foreground service start failed, falling back to startservice: %s', err.message)
            return runAm('startservice', true)
              .catch(function(innerErr) {
                return maybeRetryWithoutUser(innerErr, 'startservice')
              })
          }
          throw err
        })
