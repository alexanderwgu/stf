...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, minicap, display, screenOptions) {
    ...
    function FrameProducer(config, grabber) {
      ...
      this.projectionLocked = /^emulator-/.test(options.serial)
    ...
    FrameProducer.prototype.updateProjection = function(width, height) {
      // Emulator webviews (e.g. youtube.com in vertical mode) can emit frequent
      // size changes that cause repeated minicap restarts and visible flicker.
      // Keep a stable projection for emulators once streaming is active.
      if (this.projectionLocked && this.runningState !== FrameProducer.STATE_STOPPED) {
        log.info(
          'Ignoring projection update %dx%d for emulator %s to keep stream stable'
        , width
        , height
        , options.serial
        )
