...
module.exports = function(options) {
  ...
  function appendLogTail(taskId, chunk) {
    ...
    // Debounce DB writes — flush at most every 2 seconds instead of every chunk
    if (!logFlushTimers.has(taskId)) {
      logFlushTimers.set(taskId, setTimeout(function() {
        logFlushTimers.delete(taskId)
        const tail = logTailByTask.get(taskId)
        if (tail != null) {
          dbapi.updateAiTask(taskId, {logTail: tail}).catch(function(err) {
            log.warn('Failed to flush log tail for task %s: %s', taskId, err.message || err)
          })
        }
      }, 2000))
    }
  ...
  function flushLogTail(taskId) {
    var timer = logFlushTimers.get(taskId)
    if (timer) {
      clearTimeout(timer)
      logFlushTimers.delete(taskId)
    }
    var tail = logTailByTask.get(taskId)
    if (tail != null) {
      return dbapi.updateAiTask(taskId, {logTail: tail}).catch(function(err) {
        log.warn('Failed to flush final log tail for task %s: %s', taskId, err.message || err)
      })
    }
  ...
  function startTask(task) {
    ...
    .then(function(result) {
      ...
      return flushLogTail(task.id).then(function() {
        return result
      })
    })
    .then(function(result) {
  ...
  function startWatcher() {
    // First, pick up any tasks that were left queued from a previous run
    return db.run(
      r.table('ai_tasks').filter({status: 'queued'}),
      function(err, cursor) {
        if (err) {
          throw err
        }
    ...
    .then(function(queuedTasks) {
      queuedTasks.forEach(function(task) {
        handleChange({new_val: task})
      })
      // Then watch for new changes only (no includeInitial)
      return db.run(r.table('ai_tasks').changes(), function(err, cursor) {
        if (err) {
          throw err
        }
        return cursor
