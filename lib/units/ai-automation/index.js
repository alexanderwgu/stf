...
module.exports = function(options) {
  ...
  const MEMORY_SUMMARY_SYSTEM_PROMPT = [
    'You are a summarizer for mobile automation task memory.',
    'Return compact JSON only, no markdown and no extra text.',
    'Focus on actionable context for the next task.',
  ...
  function summarizeMemoryWithLlm(task, result, relevantLog, previousMemory) {
    ...
    const userPrompt = [
      'Produce memory for the same device for the next task.'
    , `Task status: ${status}`
    , `Task prompt: ${limitText(task.prompt, 500)}`
    , `Error summary: ${limitText(fallbackError, 300)}`
    , ''
    ...
    , ''
    , 'Return JSON with shortTermSummary and longTermFacts only.'
    ].join('\n')
  ...
  function buildMemoryUpdate(task, result, logTail, previousMemory, llmSummary) {
    ...
    if (shortLog) {
      parts.push('Relevant execution notes: ' + shortLog)
    }
