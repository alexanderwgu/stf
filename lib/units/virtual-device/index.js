...
module.exports = function(options) {
  ...
  function buildEmulatorArgs(current) {
    const args = [
      '-avd', current.avdName
    ...
    const extra = (options.emulatorArgs || []).slice()
    const hasGpuArg = extra.indexOf('-gpu') !== -1
    const hasVulkanDisable = extra.indexOf('-feature') !== -1 && extra.indexOf('-Vulkan') !== -1

    // Prefer software GPU for stable minicap capture on emulators.
    if (!hasGpuArg) {
      extra.push('-gpu', 'swiftshader_indirect')
    }
    // Vulkan composition can produce black/flickery captures with virtual displays.
    if (!hasVulkanDisable) {
      extra.push('-feature', '-Vulkan')
    }
  ...
  function configureDisplayForCapture(serial) {
    // Emulator video layers can bypass framebuffer capture and appear black/flickery in STF.
    // Disabling HW overlays forces GPU composition into capturable buffers.
    const disableOverlaysArgs = ['-s', serial, 'shell', 'settings', 'put', 'global', 'disable_hw_overlays', '1']
    const disableAnimations = [
      ['-s', serial, 'shell', 'settings', 'put', 'global', 'window_animation_scale', '0'],
      ['-s', serial, 'shell', 'settings', 'put', 'global', 'transition_animation_scale', '0'],
      ['-s', serial, 'shell', 'settings', 'put', 'global', 'animator_duration_scale', '0']
    ]

    return runAdb(disableOverlaysArgs)
      .then(function() {
        log.info('Disabled HW overlays for %s', serial)
      })
      .catch(function(err) {
        log.warn('Failed to disable HW overlays for %s: %s', serial, createActionError(err))
      })
      .then(function() {
        return Promise.each(disableAnimations, function(args) {
          return runAdb(args).catch(function() {
            return null
          })
        })
      })
  ...
  function startEmulator(record) {
    ...
    .then(function(updated) {
      const current = updated || record
      const args = buildEmulatorArgs(current)
      ...
      .then(function() {
        return configureDisplayForCapture(current.serial)
      })
      .then(function() {
