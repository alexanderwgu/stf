...
module.exports = function(options) {
  ...
  function reserveEmulatorPort() {
    return db.run(r.table('virtual_devices').pluck('emulatorPort'))
      .then(function(cursor) {
        return cursor.toArray()
      })
      .then(function(list) {
        const used = new Set(list.map(item => item.emulatorPort).filter(Boolean))
        let port = options.portMin
        if (port % 2 !== 0) {
          port += 1
        }
        while (used.has(port)) {
  ...
  function startEmulator(record) {
    return Promise.resolve().then(function() {
      ...
      if (!record.emulatorPort) {
        return reserveEmulatorPort().then(function(port) {
          return dbapi.updateVirtualDevice(record.id, {
            emulatorPort: port
      ...
      if (!record.serial && record.emulatorPort) {
        return dbapi.updateVirtualDevice(record.id, {
          serial: `emulator-${record.emulatorPort}`
        })
      }
      return record
    })
